...
func (e *Emulator) Resize(cols, rows int) {
	...
	if cols == e.cols && rows == e.rows {
		// Same size — Draw calls this every frame; skip the grid realloc and
		// scrollback re-pad. Only do the full resize on a real size change.
		return
	}
...
func (e *Emulator) SnapshotAt(offset int) Snapshot {
	...
	// Resolve the visible window directly from scrollback + live cells.
	// Copying ALL scrollback rows here first and then discarding all but the
	// visible ones was the main per-frame allocation hotspot (this ran on
	// every Draw, i.e. on every PTY chunk and every selection drag tick).
	total := len(e.scrollback) + len(e.cells)
	start := max(0, total-e.rows-offset)
	end := min(total, start+e.rows)
	...
	for y := range e.rows {
		...
		var src []Cell
		if srcIdx < len(e.scrollback) {
			src = e.scrollback[srcIdx]
		} else {
			src = e.cells[srcIdx-len(e.scrollback)]
		}
		copy(rows[y], src)
		// Rows narrower than the current width (e.g. pre-resize scrollback)
		// are padded with blanks, matching the previous behavior.
		for x := len(src); x < e.cols; x++ {
			rows[y][x] = blank
		}
	}
...
// WindowTitle returns the OSC-set window title without a full Snapshot.
...
// FocusReporting reports whether the remote app enabled focus in/out events.
func (e *Emulator) FocusReporting() bool {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.focusReporting
}

// Dimensions returns visible cols, visible rows, and scrollback row count
// without copying any cell data. Use this instead of Snapshot() when only
// geometry is needed (selection hit-testing, scroll clamps).
func (e *Emulator) Dimensions() (cols, rows, scrollbackRows int) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.cols, e.rows, len(e.scrollback)
}

// NormalizeCol clamps col into the row at absRow (scrollback-first absolute
// index) and backs it off the continuation cells of wide runes. It returns
// the normalized column, the total row count (scrollback + live), and whether
// any rows exist. Reads one row only — no allocation.
func (e *Emulator) NormalizeCol(absRow, col int) (normCol, totalRows int, ok bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	total := len(e.scrollback) + len(e.cells)
	if total == 0 {
	...
	absRow = clamp(absRow, 0, total-1)
	var row []Cell
	if absRow < len(e.scrollback) {
		row = e.scrollback[absRow]
	} else {
		row = e.cells[absRow-len(e.scrollback)]
	}
	if len(row) == 0 {
		return 0, total, true
	}
	...
	return col, total, true
}

// RowsInRange returns copies of the absolute rows [startRow, endRow]
// (clamped, order-normalized). Selection extraction uses this so copying
// cost is proportional to the selection size, not the whole scrollback.
func (e *Emulator) RowsInRange(startRow, endRow int) [][]Cell {
	e.mu.RLock()
	defer e.mu.RUnlock()
	total := len(e.scrollback) + len(e.cells)
	if total == 0 {
	...
	startRow = clamp(startRow, 0, total-1)
	endRow = clamp(endRow, 0, total-1)
	if startRow > endRow {
		startRow, endRow = endRow, startRow
	}
	out := make([][]Cell, endRow-startRow+1)
	for i := range out {
		idx := startRow + i
		var src []Cell
		if idx < len(e.scrollback) {
			src = e.scrollback[idx]
		} else {
			src = e.cells[idx-len(e.scrollback)]
		}
		copied := make([]Cell, len(src))
		copy(copied, src)
		out[i] = copied
	}
	return out
